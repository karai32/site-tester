  ...
  async run({ url }) {
    ...
    try {
      ...
      if (await link.count() === 0) {
        return { id: this.id, title: this.title, pageUrl: url, status: 'failed', message: `На странице не найдена кнопка WhatsApp (${selectors.whatsappLink}).` };
      }

      const href = await link.getAttribute('href');
      if (!href || !href.includes(expectedPhoneDigits)) {
        return { id: this.id, title: this.title, pageUrl: url, status: 'failed', message: `Ссылка WhatsApp (${href || 'пусто'}) не содержит ожидаемый номер ${expectedPhoneDigits}.` };
      }
      ...
      if (status < 200 || status >= 400) {
        return { id: this.id, title: this.title, pageUrl: url, status: 'failed', message: `Ссылка WhatsApp (${href}) вернула HTTP ${status}.` };
      }
